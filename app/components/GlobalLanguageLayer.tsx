...
export default function GlobalLanguageLayer() {
  ...
  async function translateDocument(targetLocale: AridonLocale, run: number) {
    ...
    for (const element of Array.from(document.querySelectorAll('*'))) {
      ...
      const rendered = lastRenderedAttrs.current.get(element) || {};
      let touched = false;
  ...
  return (
    <div
      ...
      style={{
        position: 'fixed', right: 14, bottom: 14, zIndex: 2147483000, display: 'flex', alignItems: 'center', gap: 8,
        padding: '8px 10px', borderRadius: 14, background: 'rgba(7,16,29,.94)', border: '1px solid rgba(158,240,207,.32)',
        boxShadow: '0 12px 34px rgba(0,0,0,.28)', color: '#fff', fontFamily: 'Arial, sans-serif',
      }}
    >
      ...
      <select
        ...
        style={{ border: 0, background: 'transparent', color: '#fff', fontWeight: 800, fontSize: 13, outline: 'none', maxWidth: 175 }}
      >
