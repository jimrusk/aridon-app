...
const severityStyle: Record<Finding['severity'], React.CSSProperties> = {
...
export default function GridIntelligencePage() {
  ...
  const counts = useMemo(() => ({ high: findings.filter(f => f.severity === 'High').length, medium: findings.filter(f => f.severity === 'Medium').length }), []);
    ...
    <main style={s.main}>
      <header style={s.header}>
        <div style={s.headerInner}>
          ...
          <div style={s.productTitle}><Drone size={18}/> Drone Grid Intelligence</div>
      ...
      <section style={s.hero}>
        ...
        <div style={s.heroDiagram}>
          <FlowStep icon={<Drone/>} label="Drone" sub="RGB · Thermal · LiDAR" />
          <Arrow />
      ...
      <section style={s.wrap}>
        ...
        <div style={s.twoCol}>
          <section style={s.panel}>
            ...
            <div style={s.missionTop}>
              <div>
                <div style={s.missionName}>Feeder 17 · North Corridor</div>
                <div style={s.muted}>Dock A → 11 inspection zones → Dock B</div>
              </div>
              <span style={s.ready}><Radio size={14}/> READY</span>
            </div>
            <div style={s.routeGraphic}>
              <RouteNode n="A" label="Dock" active />
              <RouteLine />
              <RouteNode n="01" label="Substation" />
              <RouteLine />
              <RouteNode n="06" label="Midpoint" />
              <RouteLine />
              <RouteNode n="11" label="ROW" alert />
              <RouteLine />
              <RouteNode n="B" label="Dock" active />
            </div>
          ...
          <section style={s.panel}>
            ...
            <div style={s.sampleBox}>
              <div><strong>Sample:</strong> TX-09-441</div>
              <div>Thermal: 91.4°C · baseline: 67.8°C</div>
              <div>Delta: +23.6°C</div>
            </div>
            {analysis ? (
              ...
              <div style={s.analysisBox}>
                ...
                <div style={s.reasonList}>{analysis.reasons?.map((r:string) => <span key={r}>• {r}</span>)}</div>
              </div>
          ...
          <div style={s.tableWrap}>
            <table style={s.table}>
              ...
              <tbody>{findings.map(f => <tr key={f.id}>
                <td style={s.td}><strong>{f.assetId}</strong><div style={s.small}>{f.assetType}</div></td>
                <td style={s.td}>{f.issue}</td>
                <td style={s.td}><span style={{...s.severity, ...severityStyle[f.severity]}}>{f.severity}</span></td>
                <td style={s.td}>{f.confidence}%</td>
                <td style={s.td}><ScoreBar value={f.score}/></td>
                <td style={s.td}>{f.action}</td>
              </tr>)}</tbody>
            </table>
          </div>
        </section>
        ...
        <section style={s.panel}>
          ...
          <div style={s.integrationGrid}>
            <Integration from="DJI / Autel / enterprise UAS" to="Aridon ingestion" text="Mission metadata, telemetry, RGB frames, thermal frames and LiDAR files." />
        ...
        <section style={s.panel}>
          <PanelTitle icon={<Drone size={19}/>} title="Pilot hardware package" kicker="Reference configuration" />
        ...
        <section style={s.bottomCta}>
          <div><div style={s.eyebrow}>ARIDON GRID INTELLIGENCE MVP</div><h2 style={s.h2}>Inspect → detect → prioritize → repair → verify.</h2><p style={s.muted}>The software slice is now structured for real drone feeds and utility integrations. Next step is connecting one pilot drone, one corridor and one utility asset export.</p></div>
...
function FlowStep({icon,label,sub}:{icon:React.ReactNode;label:string;sub:string}){return <div style={s.flowStep}><div style={s.flowIcon}>{icon}</div><strong>{label}</strong><span>{sub}</span></div>}
function Arrow(){return <div style={s.flowArrow}>→</div>}
function Kpi({icon,label,value,sub}:{icon:React.ReactNode;label:string;value:string;sub:string}){return <div style={s.kpi}><div style={s.kpiIcon}>{icon}</div><div style={s.kpiLabel}>{label}</div><div style={s.kpiValue}>{value}</div><div style={s.muted}>{sub}</div></div>}
function PanelTitle({icon,title,kicker}:{icon:React.ReactNode;title:string;kicker:string}){return <div style={s.panelTitle}><div style={s.panelIcon}>{icon}</div><div><div style={s.panelKicker}>{kicker}</div><h2 style={s.panelHeading}>{title}</h2></div></div>}
...
function Mini({icon,label,value}:{icon:React.ReactNode;label:string;value:string}){return <div style={s.mini}><span>{icon}</span><div><div style={s.small}>{label}</div><strong>{value}</strong></div></div>}
...
function Feature({icon,title,text}:{icon:React.ReactNode;title:string;text:string}){return <section style={s.feature}><div style={s.featureIcon}>{icon}</div><h3 style={s.featureTitle}>{title}</h3><p style={s.muted}>{text}</p></section>}
...
const s: Record<string, React.CSSProperties> = {
  main:{minHeight:'100vh',background:'#06101B',color:'#F7FAFC',fontFamily:'Arial, sans-serif'},
  header:{position:'sticky',top:0,zIndex:20,background:'rgba(6,16,27,.94)',backdropFilter:'blur(12px)',borderBottom:'1px solid #1E3144'},
  headerInner:{maxWidth:1240,margin:'0 auto',padding:'15px 20px',display:'flex',alignItems:'center',gap:18,justifyContent:'space-between',flexWrap:'wrap'},
  brand:{color:'#9EF0CF',textDecoration:'none',fontWeight:950,letterSpacing:2},
  productTitle:{display:'flex',gap:8,alignItems:'center',fontWeight:900},
  demoBadge:{display:'flex',gap:6,alignItems:'center',fontSize:11,fontWeight:900,color:'#FFD77D',border:'1px solid #6B5827',background:'#2C2612',padding:'7px 9px',borderRadius:999},
  hero:{maxWidth:1240,margin:'0 auto',padding:'58px 20px 34px',display:'grid',gridTemplateColumns:'minmax(0,1.05fr) minmax(320px,.95fr)',gap:28,alignItems:'center'},
  heroCopy:{maxWidth:720},eyebrow:{color:'#9EF0CF',fontSize:12,fontWeight:950,letterSpacing:1.2},h1:{fontSize:'clamp(52px,8vw,92px)',lineHeight:.92,letterSpacing:-4,margin:'12px 0 20px'},
  lead:{fontSize:20,lineHeight:1.65,color:'#B8C7D7',maxWidth:780},buttonRow:{display:'flex',gap:10,flexWrap:'wrap',marginTop:24},
  primaryButton:{border:0,borderRadius:12,padding:'13px 16px',background:'#9EF0CF',color:'#07130F',fontWeight:950,cursor:'pointer',display:'flex',alignItems:'center',gap:8,fontSize:14},
  secondaryButton:{border:'1px solid #3A526B',borderRadius:12,padding:'13px 16px',background:'#0B1A29',color:'#F7FAFC',fontWeight:900,cursor:'pointer',fontSize:14},
  disclaimer:{color:'#7F93A7',fontSize:12,lineHeight:1.55,maxWidth:720,marginTop:14},
  heroDiagram:{background:'#091827',border:'1px solid #233A50',borderRadius:22,padding:18,display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,overflowX:'auto'},
  flowStep:{minWidth:112,display:'flex',flexDirection:'column',gap:5,alignItems:'center',textAlign:'center'},flowIcon:{width:52,height:52,border:'1px solid #2B4B61',background:'#0D2433',borderRadius:15,display:'grid',placeItems:'center',color:'#9EF0CF'},flowArrow:{color:'#5D7891',fontSize:25,fontWeight:950},
  wrap:{maxWidth:1240,margin:'0 auto',padding:'10px 20px 70px'},kpiGrid:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:12,margin:'16px 0 12px'},
  kpi:{background:'#0A1725',border:'1px solid #20364A',borderRadius:16,padding:18},kpiIcon:{color:'#9EF0CF'},kpiLabel:{fontSize:12,fontWeight:900,color:'#8EA2B5',textTransform:'uppercase',marginTop:10},kpiValue:{fontSize:34,fontWeight:950,letterSpacing:-1,margin:'5px 0'},
  muted:{color:'#8FA3B7',fontSize:14,lineHeight:1.55},small:{color:'#8196A9',fontSize:11,marginTop:3},twoCol:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(330px,1fr))',gap:12,margin:'12px 0'},
  ...
  missionTop:{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center'},missionName:{fontSize:18,fontWeight:950},ready:{display:'flex',gap:6,alignItems:'center',fontSize:11,fontWeight:950,color:'#9EF0CF',border:'1px solid #285B50',padding:'6px 8px',borderRadius:999},
  routeGraphic:{display:'flex',alignItems:'flex-start',justifyContent:'center',margin:'28px 0 22px',overflowX:'auto',paddingBottom:5},routeNodeWrap:{display:'flex',flexDirection:'column',alignItems:'center',minWidth:58},routeNode:{width:40,height:40,borderRadius:999,border:'2px solid #52687D',display:'grid',placeItems:'center',fontWeight:950,fontSize:11},routeLabel:{fontSize:9,color:'#8096AA',marginTop:6},routeLine:{height:2,background:'#334D64',width:55,marginTop:20,flex:'0 0 55px'},
  missionStats:{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8},mini:{background:'#08131F',border:'1px solid #1A3146',borderRadius:12,padding:10,display:'flex',gap:8,alignItems:'center'},
