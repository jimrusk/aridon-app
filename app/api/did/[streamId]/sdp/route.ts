...
// POST /api/did/[streamId]/sdp — send WebRTC SDP answer back to D-ID
export async function POST(req: NextRequest, { params }: { params: { streamId: string } }) {
  ...
  try {
    ...
    for (const auth of didAuthCandidates(k)) {
      const res = await fetch(`${DID}/talks/streams/${params.streamId}/sdp`, {
        method: 'POST',
    ...
    return NextResponse.json({ error: 'all auth formats rejected for SDP' }, { status: 401 });
  } catch (e: any) {
