...
function extractText(data: ResponsePayload) {
...
function parseJson(value: string) {
  ...
  if (first < 0 || last < first) throw new Error('Evaluator returned no JSON object.');
  return JSON.parse(clean.slice(first, last + 1));
}
async function runModel(apiKey: string, model: string, input: string, maxOutputTokens: number) {
  const response = await fetch(RESPONSES_URL, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model, input, max_output_tokens: maxOutputTokens, store: false }), cache: 'no-store' });
  ...
  if (!output) throw new Error('AI service returned no readable output.');
  return output;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const scenarioId = text(body?.scenarioId, 80);
    const customPrompt = text(body?.customPrompt, 6000);
    ...
    const scenario = benchmarkScenarios.find((item) => item.id === scenarioId);
    ...
    const evaluatorModel = process.env.BENCHMARK_EVALUATOR_MODEL?.trim() || model;
    ...
    const evaluationPrompt = `Act as a strict business-system evaluator. Score only the visible candidate answer against the rubric. Do not reward confident wording without substance. Each dimension gets an integer score from 0 to 100. Return ONLY valid JSON with this exact shape: {"scores":{"accuracy":0,"judgment":0,"crossFunctional":0,"execution":0,"evidence":0,"risk":0,"control":0,"clarity":0},"strengths":["..."],"failures":["..."],"verdict":"..."}.\n\nRUBRIC:\n${rubric}\n\nSCENARIO:\n${prompt}\n\nCANDIDATE ANSWER:\n${answer}`;
    const evaluationText = await runModel(apiKey, evaluatorModel, evaluationPrompt, 1200);
    const evaluation = parseJson(evaluationText);
    const scores = evaluation.scores || {};
    let weighted = 0;
    for (const dimension of benchmarkDimensions) {
      const score = Math.max(0, Math.min(100, Number(scores[dimension.id]) || 0));
      scores[dimension.id] = score;
      weighted += score * (dimension.weight / 100);
    }

    return NextResponse.json({
      scenario: { id: scenario?.id || 'custom', title: scenario?.title || 'Custom benchmark', prompt },
      model,
      evaluatorModel,
      ...
      failures: Array.isArray(evaluation.failures) ? evaluation.failures.slice(0, 6) : [],
      verdict: text(evaluation.verdict, 1200),
      disclosure: 'Internal reproducible evaluation. This result does not claim superiority over another product unless that product is run on the same scenario and scored under the same rubric.',
    }, { headers: NO_STORE });
  } catch (error) {
    console.error('Benchmark error', error);
    return NextResponse.json({ error: error instanceof Error ? error.message : 'The benchmark run could not be completed.' }, { status: 500, headers: NO_STORE });
  }
}
