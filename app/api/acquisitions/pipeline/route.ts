...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ leads: enriched, metrics }, { headers: NO_STORE });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (lead.seller_email && !/^\S+@\S+\.\S+$/.test(lead.seller_email)) {
      return NextResponse.json({ error: 'Seller email is not valid.' }, { status: 400, headers: NO_STORE });
    }

    const db = getServerClient();
    const score = scoreAcquisition(lead);
    const structures = buildDealStructures(lead);
    const { data: created, error } = await db.from('acquisition_leads').insert({ ...lead, updated_at: new Date().toISOString() }).select('*').single();
    if (error) throw error;
    ...
    await db.from('acquisition_timeline').insert({
      ...
      event_title: 'Lead captured',
      event_detail: `${created.business_name} entered the Aridon 3 acquisition pipeline with a ${score.overall_score}/100 score.`,
      created_by: 'Aridon 3',
    });

    return NextResponse.json({ lead: created, score, structures }, { status: 201, headers: NO_STORE });
  } catch (error) {
  ...
  try {
    ...
    if (requestedStage && before?.stage !== requestedStage) {
      await db.from('acquisition_timeline').insert({
        ...
        event_title: 'Pipeline stage changed',
        event_detail: `${before?.stage || 'unknown'} → ${requestedStage}`,
        created_by: 'Owner',
      });
    }
    if (text(body?.timeline_note, 2000)) {
      await db.from('acquisition_timeline').insert({
        lead_id: id,
        event_type: 'note',
        event_title: 'Deal note added',
        event_detail: text(body.timeline_note, 2000),
        created_by: 'Owner',
      });
    }
