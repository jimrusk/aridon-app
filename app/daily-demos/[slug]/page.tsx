...
export default function DailyProspectDemoPage({ params }: Props) {
  const demo = [...dailyProspectDemos, ...prospectDemosAug11, ...prospectDemosAug13].find((item) => item.slug === params.slug);
    ...
    <main style={{ minHeight:'100vh', background:'#F4F0E8', color:'#171717', fontFamily:'Inter, ui-sans-serif, system-ui, sans-serif' }}>
      <div style={{ maxWidth:1120, margin:'0 auto', padding:'24px 20px 70px' }}>
        <header style={{ display:'flex', justifyContent:'space-between', gap:16, alignItems:'center', flexWrap:'wrap', marginBottom:24 }}>
          <Link href="/business-os/revenue-recovery" style={{ color:'#171717', textDecoration:'none', fontWeight:950, letterSpacing:'.04em' }}>ARIDON · REVENUE RECOVERY PILOT</Link>
          <span style={{ fontSize:12, fontWeight:850, background:'#FFF4D6', border:'1px solid #E4C978', borderRadius:999, padding:'7px 11px' }}>UNOFFICIAL DEMONSTRATION · TEMPORARY</span>
        </header>
        <section style={{ background:'#171717', color:'#fff', borderRadius:24, padding:'clamp(24px,5vw,54px)', marginBottom:18 }}>
          <div style={{ color:'#9EF0CF', fontSize:12, fontWeight:950, letterSpacing:'.1em' }}>14 DAYS · NO UPFRONT FEE · HUMAN APPROVAL STAYS IN CONTROL</div>
          <h1 style={{ fontSize:'clamp(38px,6vw,72px)', lineHeight:.98, margin:'16px 0 18px', maxWidth:880 }}>Where could {demo.companyName} already be leaving money or time on the table?</h1>
          <p style={{ color:'#C8D0D8', fontSize:18, lineHeight:1.65, maxWidth:800 }}>{demo.publicSummary}</p>
          <p style={{ color:'#AAB5C3', lineHeight:1.6, maxWidth:820 }}>Aridon created this unofficial demonstration from public information only. No private systems, customer records, inboxes, CRM data or accounts were accessed.</p>
          <div style={{ display:'flex', flexWrap:'wrap', gap:10, marginTop:20 }}><Link href="/business-os/revenue-recovery" style={{ background:'#9EF0CF', color:'#07130F', textDecoration:'none', borderRadius:10, padding:'12px 15px', fontWeight:950 }}>See the 14-Day Pilot</Link><a href={demo.website} target="_blank" rel="noreferrer" style={{ border:'1px solid #465164', color:'#fff', textDecoration:'none', borderRadius:10, padding:'12px 15px', fontWeight:850 }}>Public source website</a></div>
        </section>
