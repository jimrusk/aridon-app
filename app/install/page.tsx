    ...
    <main style={{ minHeight: '100vh', background: '#07101D', color: '#F8FAFC', fontFamily: 'Arial, sans-serif' }}>
      <section style={{ maxWidth: 1000, margin: '0 auto', padding: '30px 20px 80px' }}>
        <nav style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Link href="/business-os/subscribe" style={{ color: mint, textDecoration: 'none', fontWeight: 900 }}>See subscription</Link>
        </nav>
        ...
        <div style={{ paddingTop: 64, maxWidth: 900 }}>
          ...
          <p style={{ color: '#B8C4D5', fontSize: 20, lineHeight: 1.65, maxWidth: 820 }}>Aridon can now install on supported computers, phones and tablets as a standalone company app. The same private workspace follows the company account rather than living on one machine.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 14, marginTop: 30 }}>
          ...
          <section style={{ background: '#F4F1E9', color: '#171717', borderRadius: 20, padding: 26 }}>
            ...
            <h2 style={{ fontSize: 34, margin: '8px 0 12px' }}>$497/month founding plan</h2>
            <ol style={{ color: '#5D5A54', lineHeight: 1.9, paddingLeft: 22 }}>
              <li>Start the Aridon company subscription.</li>
              <li>Open the company workspace with the authorized account.</li>
              <li>Install Aridon on each supported device the team uses.</li>
            </ol>
            <a href={CHECKOUT_URL} target="_blank" rel="noreferrer" style={{ display: 'inline-block', background: '#171717', color: '#fff', textDecoration: 'none', fontWeight: 950, padding: '14px 18px', borderRadius: 12 }}>Buy Aridon for My Company</a>
          </section>
        </div>
        ...
        <section style={{ marginTop: 16, border: '1px solid #2A3A57', borderRadius: 20, padding: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 12 }}>
            <Feature title="Windows & Mac" text="Run Aridon in a dedicated app window from supported desktop browsers." />
