...
export default function CloudWorkersLauncher() {
  const router = useRouter();
  const [message, setMessage] = useState('Opening Eva Cloud Workers…');
  ...
  useEffect(() => {
    ...
    db.auth.getSession().then(async ({ data }) => {
      ...
      if (!token) {
        router.replace('/customer/login?next=/cloud-workers');
        return;
      }
      ...
      router.replace(`/workspace/${result.tenant.slug}/cloud-workers`);
    });
  }, [router]);
