...
import { BarChart3, CheckCircle2, FileCheck2, HandCoins, Landmark, LineChart, LockKeyhole, Save, ShieldCheck } from "lucide-react";
...
export default function RegenerativeUnderwritingPage(){
  ...
  const capitalPlan=useMemo(()=>{
    ...
    if(projectType==='Land access') return [
      ['Producer equity',equity,'Owner commitment / first-loss alignment'],
      ['Land capital / long-term lease',Math.max(0,gap*.75),'Patient farmland or lease capital'],
      ['Conservation / transition assistance',grant,'Public or philanthropic cost share'],
      ['Bridge / working capital',Math.max(0,gap*.25),'Transition-period liquidity'],
    ];
    if(projectType==='Aggregator / supply chain') return [
      ['Producer / sponsor equity',equity,'Sponsor commitment'],
      ['Grant / catalytic capital',grant,'De-risk early capacity and market development'],
      ['Working-capital facility',Math.max(0,gap*.55),'Inventory / receivables / seasonal needs'],
      ['Growth debt or equity',Math.max(0,gap*.45),'Expansion and aggregation infrastructure'],
    ];
    return [
      ['Producer equity',equity,'Owner commitment'],
      ['Grant / cost share',grant,'NRCS, USDA, state, foundation or corporate support'],
      ['Equipment / project debt',Math.max(0,gap*.7),'Cash-flow-supported senior capital'],
      ['Impact / subordinated capital',Math.max(0,gap*.3),'Fill transition or risk-sharing gap'],
    ];
  ...
  async function saveWorkspace(){
    ...
    try{
      ...
      const id=project.id as string; setProjectId(id);
      ...
      await supabase.from('regenerative_risk_assessments').insert(risk);
      await supabase.from('regenerative_capital_stack').delete().eq('project_id',id);
      await supabase.from('regenerative_capital_stack').insert(capitalPlan.map(([type,amount,why])=>({project_id:id,capital_type:String(type),amount:Number(amount),status:'candidate',conditions:{rationale:String(why)}})));
      setStatus('Saved. Aridon now has the project, risk assessment and coordinated capital stack.');
    }catch(e:any){setStatus(`Save failed: ${e?.message||'unknown error'}`);}
  }

  async function addMeasurement(){
    const m={...measurement}; setMeasurements(v=>[m,...v]);
    if(!projectId){setStatus('Measurement added locally. Save the workspace to persist it.');return;}
    try{
      const supabase=getBrowserClient();
      const {error}=await supabase.from('regenerative_measurements').insert({project_id:projectId,category:m.category,metric:m.metric,value:m.value,unit:m.unit,source:m.source,verification_status:m.verified?'verified':'unverified',confidence:m.verified?90:60});
      if(error) throw error;
      setStatus('Measurement added to the verified outcomes ledger.');
  ...
  async function approveDisclosure(){
    ...
    if(!recipient.trim()){setStatus('Enter a lender, insurer or investor name first.');return;}
    try{
      const supabase=getBrowserClient();
      const {error}=await supabase.from('regenerative_disclosures').insert({project_id:projectId,recipient_name:recipient,recipient_category:'capital / risk partner',purpose:'regenerative underwriting review',approved_fields:['project economics','capital stack','risk scores','verified measurements']});
      if(error) throw error;
  ...
  return <main style={{minHeight:'100vh',background:'#f4f1e8',color:'#18251d',fontFamily:'Arial,sans-serif'}}>
    ...
    <section style={{maxWidth:1180,margin:'auto',padding:'46px 18px 28px'}}><div style={{color:'#356943',fontSize:12,fontWeight:950,letterSpacing:1.2}}>ARIDON REGENERATIVE CAPITAL COORDINATOR</div><h1 style={{fontSize:'clamp(40px,6vw,70px)',lineHeight:.98,margin:'8px 0 14px'}}>From farm change to finance-ready evidence.</h1><p style={{fontSize:19,lineHeight:1.6,color:'#56635a',maxWidth:900}}>Identify the actual constraint, coordinate grants, land capital, infrastructure finance, debt and impact capital, then connect soil, water, production, insurance and financial evidence to a lender-ready risk profile.</p></section>
    ...
    <section style={{maxWidth:1180,margin:'auto',padding:'0 18px 40px',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,360px),1fr))',gap:16}}>
      <article style={{background:'#fff',border:'1px solid #d6dfd3',borderRadius:18,padding:20}}><h2 style={{marginTop:0}}>1. Define the blockage</h2><div style={{display:'grid',gap:12}}><label style={{fontWeight:800,fontSize:13}}>Operation<input value={operation} onChange={e=>setOperation(e.target.value)} placeholder="Ranch or farm name" style={{display:'block',width:'100%',boxSizing:'border-box',marginTop:6,padding:11,border:'1px solid #b9c6bb',borderRadius:10}}/></label><label style={{fontWeight:800,fontSize:13}}>State<input value={stateName} onChange={e=>setStateName(e.target.value)} style={{display:'block',width:'100%',boxSizing:'border-box',marginTop:6,padding:11,border:'1px solid #b9c6bb',borderRadius:10}}/></label>{field('Acres',acres,setAcres)}<label style={{fontWeight:800,fontSize:13}}>Project type<select value={projectType} onChange={e=>setProjectType(e.target.value as ProjectType)} style={{display:'block',width:'100%',marginTop:6,padding:11,border:'1px solid #b9c6bb',borderRadius:10}}>{['Water / infrastructure','Land access','Soil / crop transition','Grazing / livestock','Aggregator / supply chain'].map(x=><option key={x}>{x}</option>)}</select></label><label style={{fontWeight:800,fontSize:13}}>Transition goal<textarea value={goal} onChange={e=>setGoal(e.target.value)} rows={3} style={{display:'block',width:'100%',boxSizing:'border-box',marginTop:6,padding:11,border:'1px solid #b9c6bb',borderRadius:10}}/></label></div></article>
      <article style={{background:'#fff',border:'1px solid #d6dfd3',borderRadius:18,padding:20}}><h2 style={{marginTop:0}}>2. Economics</h2><div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:12}}>{field('Project cost',projectCost,setProjectCost)}{field('Grant / cost share',grant,setGrant)}{field('Producer equity',equity,setEquity)}{field('Annual benefit',annualBenefit,setAnnualBenefit)}{field('Annual debt service',annualDebtService,setAnnualDebtService)}{field('Operating margin',margin,setMargin,'%')}</div><div style={{marginTop:16,padding:16,borderRadius:14,background:'#163d2a',color:'#fff'}}><div style={{fontSize:12,fontWeight:950,color:'#c8e2ac'}}>FINANCING GAP</div><div style={{fontSize:38,fontWeight:950}}>{money(model.debt)}</div><div>Directional DSCR {model.dscr.toFixed(2)}x · debt/project cost {(model.leverage*100).toFixed(0)}%</div></div></article>
    </section>

    <section style={{background:'#fff',borderTop:'1px solid #d6dfd3',borderBottom:'1px solid #d6dfd3',padding:'48px 18px'}}><div style={{maxWidth:1180,margin:'auto'}}><div style={{display:'flex',alignItems:'center',gap:9}}><HandCoins color="#356943"/><strong style={{color:'#356943'}}>COORDINATED CAPITAL STACK</strong></div><h2 style={{fontSize:38,margin:'8px 0 18px'}}>Finance the constraint, not just the farm.</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(230px,1fr))',gap:12}}>{capitalPlan.map(([type,amount,why])=><article key={String(type)} style={{border:'1px solid #d6dfd3',borderRadius:15,padding:17,background:'#faf9f4'}}><Landmark size={22} color="#356943"/><h3 style={{margin:'9px 0 4px'}}>{String(type)}</h3><div style={{fontSize:25,fontWeight:950}}>{money(Number(amount))}</div><p style={{color:'#5b675f',lineHeight:1.5,marginBottom:0}}>{String(why)}</p></article>)}</div></div></section>

    <section style={{maxWidth:1180,margin:'auto',padding:'50px 18px'}}><div style={{display:'flex',alignItems:'center',gap:9}}><ShieldCheck color="#356943"/><strong style={{color:'#356943'}}>REGENERATIVE RISK & UNDERWRITING</strong></div><h2 style={{fontSize:38,margin:'8px 0 8px'}}>Translate resilience into underwriting evidence.</h2><p style={{color:'#59665e',maxWidth:880,lineHeight:1.55}}>Scores are directional decision support, never a credit decision or actuarial certification. Aridon preserves the underlying evidence so lenders and insurers can apply their own models.</p><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12,margin:'22px 0'}}>{field('Years of documented soil practices',soilYears,setSoilYears)}{field('Water reliability',waterReliability,setWaterReliability,'%')}{field('Enterprise diversification',diversification,setDiversification,'/5')}{field('Loss-free / low-loss years',lossFreeYears,setLossFreeYears)}{field('Evidence/data coverage',dataCoverage,setDataCoverage,'%')}</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:10}}>{[['Soil',model.soil],['Water',model.water],['Production',model.production],['Financial',model.financial],['Climate',model.climate],['Insurance',model.insurance],['Data',model.data]].map(([n,v])=>scoreCard(String(n),Number(v)))}</div><article style={{marginTop:14,background:'#163d2a',color:'#fff',borderRadius:17,padding:20,display:'flex',justifyContent:'space-between',gap:18,flexWrap:'wrap',alignItems:'center'}}><div><small style={{color:'#c8e2ac',fontWeight:950}}>ARIDON UNDERWRITING READINESS</small><div style={{fontSize:43,fontWeight:950}}>{model.overall.toFixed(0)}/100</div></div><div style={{maxWidth:650,lineHeight:1.55}}>{model.overall>=75?'Strong evidence package. Verify source documents and partner-specific underwriting criteria before submission.':model.overall>=55?'Promising, but Aridon should close evidence or cash-flow gaps before lender/insurer review.':'Early-stage. Build measurements, strengthen economics and reduce concentrated risks before financing outreach.'}</div></article></section>

    <section style={{background:'#e6ecdf',padding:'48px 18px'}}><div style={{maxWidth:1180,margin:'auto'}}><div style={{display:'flex',alignItems:'center',gap:9}}><LineChart color="#356943"/><strong style={{color:'#356943'}}>MEASUREMENT HISTORY</strong></div><h2 style={{fontSize:36,margin:'8px 0 16px'}}>Build the actuarial and performance record over time.</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10,background:'#fff',padding:16,borderRadius:16}}><label>Category<select value={measurement.category} onChange={e=>setMeasurement({...measurement,category:e.target.value})} style={{display:'block',width:'100%',padding:10,marginTop:5}}>{['Soil','Water','Production','Economics','Insurance','Climate'].map(x=><option key={x}>{x}</option>)}</select></label><label>Metric<input value={measurement.metric} onChange={e=>setMeasurement({...measurement,metric:e.target.value})} style={{display:'block',width:'100%',boxSizing:'border-box',padding:10,marginTop:5}}/></label><label>Value<input type="number" value={measurement.value} onChange={e=>setMeasurement({...measurement,value:Number(e.target.value)||0})} style={{display:'block',width:'100%',boxSizing:'border-box',padding:10,marginTop:5}}/></label><label>Unit<input value={measurement.unit} onChange={e=>setMeasurement({...measurement,unit:e.target.value})} style={{display:'block',width:'100%',boxSizing:'border-box',padding:10,marginTop:5}}/></label><label>Source<input value={measurement.source} onChange={e=>setMeasurement({...measurement,source:e.target.value})} style={{display:'block',width:'100%',boxSizing:'border-box',padding:10,marginTop:5}}/></label><label style={{display:'flex',alignItems:'end',gap:8,paddingBottom:10}}><input type="checkbox" checked={measurement.verified} onChange={e=>setMeasurement({...measurement,verified:e.target.checked})}/> Verified source</label><button onClick={addMeasurement} style={{border:0,borderRadius:11,background:'#163d2a',color:'#fff',fontWeight:900,padding:12,cursor:'pointer'}}>Add measurement</button></div>{measurements.length>0&&<div style={{display:'grid',gap:8,marginTop:12}}>{measurements.map((m,i)=><div key={i} style={{background:'#fff',border:'1px solid #d2dccc',borderRadius:12,padding:12}}><strong>{m.category}: {m.metric}</strong> · {m.value} {m.unit} · {m.source} · {m.verified?'verified':'unverified'}</div>)}</div>}</div></section>

    <section style={{maxWidth:1180,margin:'auto',padding:'48px 18px',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:16}}><article style={{border:'1px solid #d6dfd3',borderRadius:17,padding:20,background:'#fff'}}><LockKeyhole color="#356943"/><h2>Permission-controlled packet</h2><p style={{lineHeight:1.55,color:'#5a675f'}}>Aridon can identify a lender, insurer or investor without sharing producer data. Approval is recorded before project economics, scores or measurements leave the producer workspace.</p><input value={recipient} onChange={e=>setRecipient(e.target.value)} placeholder="Recipient organization" style={{width:'100%',boxSizing:'border-box',padding:11,border:'1px solid #b9c6bb',borderRadius:10}}/><button onClick={approveDisclosure} style={{marginTop:10,border:0,borderRadius:11,background:'#163d2a',color:'#fff',padding:'12px 14px',fontWeight:900,cursor:'pointer'}}>Approve underwriting packet</button></article><article style={{background:'#163d2a',color:'#fff',borderRadius:17,padding:20}}><FileCheck2 color="#c8e2ac"/><h2>Partner-ready output</h2><p style={{color:'#dbe8df',lineHeight:1.55}}>Use of proceeds, capital stack, producer contribution, grants/cost share, DSCR, risk drivers, loss history, soil/water/production evidence, verification status and disclosure permission are assembled in one trail.</p><button onClick={()=>window.print()} style={{border:'1px solid #c8e2ac',borderRadius:10,background:'transparent',color:'#fff',padding:'11px 13px',fontWeight:900,cursor:'pointer'}}>Print / save underwriting brief</button></article></section>

    <section style={{background:'#fff',borderTop:'1px solid #d6dfd3',padding:'30px 18px 60px'}}><div style={{maxWidth:1180,margin:'auto',display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}><button onClick={saveWorkspace} style={{display:'inline-flex',alignItems:'center',gap:8,border:0,borderRadius:12,background:'#163d2a',color:'#fff',padding:'14px 17px',fontWeight:950,cursor:'pointer'}}><Save size={18}/> Save to Aridon</button><Link href="/ag/funding" style={{display:'inline-flex',alignItems:'center',gap:8,border:'1px solid #9aac9d',borderRadius:12,padding:'13px 16px',color:'#163d2a',textDecoration:'none',fontWeight:900}}><BarChart3 size={18}/> Match funding programs</Link>{status&&<div style={{display:'flex',alignItems:'center',gap:7,color:'#425248'}}><CheckCircle2 size={18}/>{status}</div>}</div></section>
  </main>;
}
